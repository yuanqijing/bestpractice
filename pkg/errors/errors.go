...
func header(depth int) string {
	...
	buf := formatHeader(file, line)
	defer bufferPool.PutBuffer(buf)
	return buf.String()
}

func formatHeader(file string, line int) *Buffer {
	buf := bufferPool.GetBuffer()
	now := time.Now()
	buf.FormatHeader(file, line, now)
	return buf
}

func (b *base) formatInfo() string {
	if b.header == "" {
		return b.info
	}
	return fmt.Sprintf("%s%s", b.header, b.info)
}

// Error implements the error interface.
func (b *base) Error() string {
	if b.err != nil {
		return fmt.Sprintf("%s\n%s", b.formatInfo(), b.err.Error())
	}
	return b.formatInfo()
}
...
func New(message interface{}) error {
	...
	default:
		info = fmt.Sprintf("%s", spew.Sdump(message))
	}
